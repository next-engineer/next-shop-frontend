...
import { register } from "@/lib/api/user" // 경로 맞게 수정
...
export default function SignupPage() {
  ...
  const validateEmail = (email: string) => {
    ...
    setValidation(prev => ({
      ...prev,
      email: emailRegex.test(email) ? { isValid: true, message: "사용 가능한 이메일입니다." } : { isValid: false, message: "올바른 이메일 형식이 아닙니다." }
    }))
  }
  const validatePassword = (password: string) => {
    ...
    setValidation(prev => ({
      ...prev,
      password: passwordRegex.test(password) ? { isValid: true, message: "사용 가능한 비밀번호입니다." } : { isValid: false, message: "영문, 숫자, 특수문자 포함 8자 이상이어야 합니다." }
    }))
  }
  const validateConfirmPassword = (confirmPassword: string) => {
    setValidation(prev => ({
      ...prev,
      confirmPassword: confirmPassword === formData.password ? { isValid: true, message: "비밀번호가 일치합니다." } : { isValid: false, message: "비밀번호가 일치하지 않습니다." }
    }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register({
        ...
        name: formData.name,
        delivery_address: JSON.stringify(formData.address),
