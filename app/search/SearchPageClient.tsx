...
// 프로젝트에 shadcn/ui가 있다면 유지하세요. 없다면 Input/Select/Button 부분을 일반 <input>/<select>/<button>으로 바꿔도 됩니다.
...
export default function SearchPageClient() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // 베이스 URL 설정 (끝 슬래시 제거)
      const baseRaw = process.env.NEXT_PUBLIC_API_BASE_URL || process.env.API_BASE_URL || ""
      const base = baseRaw.replace(/\/$/, "")
      // base가 /api로 끝나지 않으면 자동으로 /api를 붙임 (둘 다 만족 시 중복 방지)
      const apiPrefix = /\/api$/.test(base) ? "" : "/api"

      // 최종 요청 경로: .../api/products
      const res = await fetch(`${base}${apiPrefix}/products?${qs}`, { cache: "no-store" })
